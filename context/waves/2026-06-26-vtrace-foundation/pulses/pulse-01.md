...
Author `docs/vtrace/MISSION.md` — the leftmost VTRACE artifact for BASIN — and
drive it to a `.roles` review fixed point. One VTRACE deliverable only.

## Changes

- Bootstrap repo identity: `README.md`, `PRODUCT_PLAN.md`, `CLAUDE.md`,
  `AGENTS.md`, `.gitignore`.
- Establish the review panel: `.roles/` (7 parliament voices incl. the
  water-rights/transboundary realist, 3 editorial roles, 5 stakeholder lenses,
  peer panel).
...
| Basin Planner | pass | Basin-wide balance + tiering + multi-scale intent present. |
| Drought Hydrologist | pass | Hydrologic-basis framing required (NEED-002/003). |
| Environmental-Flow & Equity advocates | pass | Ecological flow + safe access first-class. |
| Water-Rights Realist | finding → resolved | Added Rights/transboundary user lens + CON-006 rights-determination boundary. |

Fixed point: 1 minor finding raised and applied; no unresolved critical/major
finding. Deferred: dimension pool, rubric, tier SLA thresholds, hydrologic method,
