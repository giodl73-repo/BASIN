...
Interface type: target (greenfield). Controls BASIN's external and cross-layer
boundaries so future work packages cannot change them silently. IF-001..004 restate
the `SPECIFICATION_BASELINE.md` public contracts; IF-005..006 add the crate API and
CLI introduced in `ARCHITECTURE.md`. None are implemented yet.

## Interface Inventory

| ID | Interface | Type | Owner | Consumers | Compatibility Rule | Verification |
|---|---|---|---|---|---|---|
| IF-001 | Corpus entry file (incl. scale/basin) | file (markdown + frontmatter) | PKG-002 | analysts, PKG-003/004/005, reviewers | Frontmatter keys additive; `id` immutable; `scale` from a fixed enum | future VER-004/016 / schema check |
...
| IF-005 | `basin-network` library API | API (Rust crate) | PKG-001 | PKG-002..006 | Public types/functions semver; breaking change is change-control | future VER-007 / cargo test |
| IF-006 | `basin` CLI | CLI | PKG-006 | maintainers, agents, analysts | Subcommands/flags additive (incl. `--scale`); output schemas versioned | future VER-001 / command review |

## Interface Details

### IF-001: Corpus entry file

Purpose: one water system/element as a reviewable, scored, labelled,
**scale-tagged** record.

Inputs: frontmatter (`id`, `type`, `scale`, `basin`, `termini`, `tier`, `sla`,
source rows) + a dimension-score block (DIM-01..13).

Outputs: a `validated`/`held`/`draft` corpus artifact joinable by `id`, filterable
by `scale`.

Errors: missing `id`, uncited quantity, or missing `scale` → held (REQ-005);
type/scope/scale drift → Scope Keeper finding.

Versioning: frontmatter keys additive; `id` semantics immutable; `scale` from a
fixed enum (international/national/regional/local); schema in `corpus/SCHEMA.md`
(deferred).
...
Inputs: tier definitions, SLA terms (supply reliability, quality, continuity,
access), element tier assignments.
...
### IF-005: `basin-network` library API

Purpose: the graph primitive every layer builds on.
Inputs: sources, nodes, conveyance segments with attributes and identity.
Outputs: graph queries — conveyance connectivity, path existence, supply/yield
helpers (with typed hydrologic basis).
Errors: typed errors for unknown/duplicate ids and bad input (no panics on expected
bad input). Versioning: semver; breaking public API change is change-control.
Evidence: future VER-007 (cargo test).

### IF-006: `basin` CLI

Purpose: orchestrate the pipeline and emit artifacts reproducibly, at a chosen
scale.
Inputs: corpus/data paths, subcommands (e.g. corpus, score, tier-sla, gap), and a
`--scale` filter.
Outputs: regenerated corpus/score/tier/gap artifacts with labels and scale
preserved.
...
| IFQ-004 | Whether `--scale` accepts a nested path (treaty/basin/local). | Defer to DEF-005. |
...
| Hydraulic Engineer | IF-005 exposes connectivity/supply as typed queries with error handling, not panics. | pass |

Fixed-point note: no actionable finding required a change; interfaces are consistent
with SPEC public contracts and architecture boundaries. No unresolved critical/major
finding. Schema, output-format, and scale-nesting details deferred to IFQ-001..004.
