...
Repo: BASIN (all `basin-*` crates)

Risk level: medium (research tooling with critical graph/identity/label/scale logic;
no safety-of-life, but wrong supply/quality/SLA claims are harmful if overstated).
...
|---|---|---|---|---|
| CR-001 | Functions stay small enough for focused review; soft cap 60 logical lines. | All `basin-*` code | review + `cargo clippy` (`too_many_lines`) | Larger units need a rationale comment. |
...
| CR-006 | No `unwrap`/`expect`/`panic!` in library code paths except tests and documented invariants. | `basin-network/corpus/score/tier/gap` libs | `cargo clippy`, review | Allowed in `basin-cli` top-level error reporting and tests. |
...
| Area | Rule | Rationale |
|---|---|---|
| Errors | Each lib crate defines a `thiserror` error enum; `basin-cli` uses `anyhow`. | Typed library errors (CR-003), ergonomic CLI. |
| Numbers | Volume = acre-feet/m³, flow = MGD/m³·s⁻¹, concentration = mg/L; never conflate; scores are `f64` in `[0,10]`. | Numeracy discipline (editorial gate). |
...
| Hydrologic basis | Supply/yield values carry their basis (firm/average) as typed data, not prose. | Enforce REQ-007/SPEC-HB-01 at the type level. |
...
| Drought Hydrologist | The hydrologic-basis tailoring rule pins firm/average into the type system, not prose. | pass |
