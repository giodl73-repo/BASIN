...
VTRACE adoption scope: define verification methods and command levels for BASIN's
requirements. BASIN is greenfield, so implementation-dependent checks are `pending`
(no code/corpus to run yet). Process-level requirements that already have evidence
(VTRACE discipline, child-repo scoping, the review panel) are verified now. A
method being named here does not mean it has run — `EVID-*` records only actual
results.
...
| REQ-004 | schema check / inspection | validate corpus frontmatter keys against schema | stable source/conveyance/system id present; labels are not keys | pending | future EVID-004 |
...
| REQ-007 | analysis / inspection | inspect supply/yield claims for named hydrologic basis | firm-yield vs average basis named on each claim | pending | future EVID-007 |
| REQ-008 | gap inspection / review | inspect an already-adequate system's gap artifact | null result recorded, no manufactured gap | pending | future EVID-008 |
...
| REQ-010 | role review | confirm supply/drought/conveyance/quality/flow/equity/cost/rights lenses represented | stakeholder lenses present in `.roles/` and applied | pass_with_risk | EVID-010 (`.roles/` panel built) |
...
| REQ-012 | git inspection | `git status --short`; confirm no TRACKER pointer touched | BASIN changes stay in the child repo | passed | EVID-012 |
...
| REQ-015 | gate / gap inspection | tier-SLA conformance (DIM-13) check on a system | tier-SLA shortfalls reported before adequacy claimed | pending | future EVID-015 |
| REQ-016 | schema check / gate | validate `scale`/`basin` tags + within-scale gap filter | every element scale-tagged; cross-scale notes explicit | pending | future EVID-016 |
...
# Implementation levels (planned — once basin-network/CLI exist)
# cargo fmt --check
# cargo test
# basin corpus regenerate ; basin tier-sla --gate ; basin gap --scale regional
```
