...
VTRACE adoption scope: describe the operating scenarios that BASIN's requirements
...
projects. BASIN is greenfield, so these scenarios describe intended operation, and
each names the mission need it serves. Every scenario is **scale-aware**.

## Actors

| Actor | Responsibility | Needs |
|---|---|---|
| BASIN maintainer | Own repo truth, active goal, generated artifacts, and promotion posture at a stated scale. | Clear commands, evidence labels, review gates, scoped child-repo changes. |
...
| Water-resources analyst | Inspect corpus scores, gap maps, and design options at a chosen scale. | Reproducible artifacts with source, confidence, evidence posture, and scale. |
...
| Stakeholder reviewer | Apply basin-planner, hydraulic, drought, economist, environmental-flow, equity, and water-rights lenses. | Claims that name supply, drought basis, flows, access, cost, and rights/transboundary posture. |

## Scenarios

### OPS-001: Add And Score An Existing System (serves NEED-001, NEED-002, NEED-004, NEED-008)

Trigger: a maintainer or agent adds an existing water system/element to the corpus.

Inputs: public source data (USGS / EPA / Reclamation / state / UN-FAO), the
dimension pool, the corpus schema, `data/sources.md`, and the element's **scale**
and basin.

Normal path:

1. Create one corpus file with a stable source/conveyance/system identifier and a
   declared `scale` (international/national/regional/local).
...
Trigger: enough systems are scored to test which dimensions differentiate.
...
1. Plot scored systems in the dimension space at the chosen scale.
2. Identify under-served regions (e.g. high supply-demand deficit + low storage +
   low drought resilience).
3. Record candidate gaps with the dimensions, corpus comparison, and scale that
   define them.

Failure or degraded path: if a system is already resilient and equitable, that null
result is recorded as a finding (NEED-006), not manufactured into a gap.
...
Trigger: a conceptual Water 2.0 project/feature package is proposed for downstream
use.
...
2. Run the 7-voice parliament; require each voice to challenge or accept —
   including the water-rights realist on any transfer/allocation assumption.
3. Run the 3-role editorial gate (citation, scope incl. scale, numeracy).
4. Promote only the bounded claim; keep construction readiness, rights
...
Trigger: a maintainer asks to advance BASIN's VTRACE baseline.
...
Trigger: a system is added or re-evaluated, or a market's adequacy is assessed.

Inputs: element attributes (source, conveyance role, service), the T1–T4 tier
model, and the per-tier SLA contract.

Normal path:

1. Classify the element into T1 (Interbasin/Transboundary Transfer), T2 (Regional
   Conveyance), T3 (Local Distribution Trunk), or T4 (Service/Last-mile).
2. Look up the tier's SLA (supply reliability, quality, continuity, access).
3. Assess conformance: does the element meet its tier SLA, with the hydrologic
   basis named (firm yield vs average, REQ-007)?
4. Record a tier-SLA gap where the element under-serves its tier promise.

Failure or degraded path: if tier or SLA inputs are missing, the element is held
...
(international/national/regional/local), and the basin/jurisdiction filter.

Normal path:

1. Select the scale and basin/jurisdiction for the run.
...
4. If a cross-scale relationship matters (e.g. a national transfer affecting a
   local system), record it as an explicit, labelled cross-scale note (CON-007).
...
- BASIN is greenfield: VTRACE is authored ahead of implementation, so scenarios
...
- Some data sources may be expensive or blocked; BASIN records intended acquisition
  and validation even when a full pass is deferred.
- `.roles` review is part of BASIN operations and must change evidence posture,
  claim labels, or next steps when it finds a gap.
- TRACKER remains the portfolio snapshot repo; BASIN owns repo-local implementation
...
|---|---|---|
| Scope Keeper | Scenarios describe workflows; no specific system/gap/design prescribed; OPS-007 enforces scale scoping. | pass |
| Citation Auditor | No new quantitative claims; scenarios name repo-local artifacts and future evidence paths. | pass |
| Numeracy Checker | No arithmetic, units, score ranges, supply, or cost figures. | pass |
| Basin Planner / Drought Hydrologist | Scoring and review scenarios require hydrologic-basis evidence before promotion. | pass |
| Water-Rights Realist | OPS-004 explicitly requires the realist to challenge transfer/allocation assumptions. | pass |
...
Fixed-point note: one actionable finding (source-label/scale custody
under-specified) was raised and applied. No unresolved critical or major finding
remains.
...
| OQ-003 | What hydrologic basis (firm yield under drought of record vs average) anchors supply scoring? | Defer to `SPECIFICATION_BASELINE.md`. |
